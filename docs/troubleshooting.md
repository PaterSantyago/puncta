...
Standalone ranges need an explicit option. Digit grouping and hyphenation insertion are off by default.
Protected text does not receive typography changes.

## A locale change keeps the previous style

Explicit values survive a locale change. Reset the relevant field or group with `null`.
...
Supply all the custom designations that the variant needs.
...
For `locale.unavailable`, load the module at creation before selecting its ID.
...
## A warning recurs without a change
