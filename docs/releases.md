...
After all planned versions match, the full npm/pnpm consumer matrix installs by name from that registry using the manifests in the retained archive set. This includes packages outside an independent locale plan, which must already exist in the registry at their checked versions. The consumer never publishes or rebuilds these packages. Only after this verification does the command create immutable local tags named `<package>@<version>` at the checked release commit. Public modes fetch remote tags before publication and reject conflicts. The owner then pushes these exact tags without force, for example:
