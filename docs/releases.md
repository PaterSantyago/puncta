...
npm can expose a newly published version and its dist-tags while the package index used by installation still returns 404. The executor reads the dedicated dist-tags endpoint and records the observed tags. It checks index visibility up to six times, at two-second intervals, before it proceeds to another package. Each request has a 30-second timeout. If the index remains unavailable, publication stops with a recoverable incomplete result; wait for npm to expose it and rerun the same bundle and state. The existing version is verified and skipped on that retry.
