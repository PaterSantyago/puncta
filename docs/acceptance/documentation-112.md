...
The settings reference defines fields, defaults, valid surfaces, inheritance, resets, and validation.
The core and React references define removal arguments, result types, and all overloads.
The locale reference defines word admission, conservative exclusions, and language evidence limits.
...
Full diagnostics remain in issue 114.
...
Source behavior does not serve as the sole language oracle.
...
Matching functional archives retain manifest version `0.1.0-alpha.0`.
...
The installed-consumer harness extracts the displayed TypeScript/TSX source and compares exact output, including SHY escapes.
No new layout claim requires a browser run.
The guide states that SHY opportunities do not require rendered line breaks.
Existing React browser evidence remains in [issue 110 acceptance](documentation-110.md#checks).

## Review and limits

The author used the official ASD-STE100 Issue 9 rules and dictionary and the established domain terms.
No project dictionary applies. A separate factual and language review is pending.
The coordinator supplies that review before issue acceptance.

The functional packages are unreleased.
Public installation checks and release/tag links remain pending until publication.
Verification of new remote URLs is pending until the coordinator pushes the commit.
