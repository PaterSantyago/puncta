...
is not an unconditional license to cut against current British pronunciation.
...
still contain explicitly labeled British pronunciations; only those British
fields are used.
...
proves data identity, not linguistic correctness. Later corrections need new independent source reasoning, an explicit review, and a new corpus version. Investigate engine differences. They do not justify removal of required positions or restoration of prohibited runtime exceptions.
...
- Negative inputs cover ASCII/curly/modifier apostrophes and ordinary, Unicode, and nonbreaking hyphens. They also cover existing SHY, short words, case, digits, protection, unsupported NFC/NFD diacritics, a ligature, and Cyrillic mixing.
...
resource or hyphenation engine. It validates identity, counts, admitted spelling, settings, distinct words, required categories, and actual length diversity. It also checks sorted positions, grapheme boundaries, minima, required containment, source/review metadata, and negative-case coverage. Deliberately damaged fixtures demonstrate
...
runtime, idempotence, Unicode preservation and diagnostic behavior remain work
